...
@router.post("/puzzles/user-games")
async def puzzles_user_games(body: FromUserMlRequest):
    """
    Load a user's recent games.

    This endpoint ONLY loads games.
    ...
    try:
        ...
        log.error(
            "[/puzzles/user-games] error: %s",
            e,
        )

        raise HTTPException(
            status_code=500,
            detail="failed to load recent games",
        )
...
async def puzzles_daily():
    try:
        ...
        log.error(
            "[/puzzles/daily] chess.com fetch failed: %s",
            e,
        )
    ...
    if not fen:
        log.warning(
            "[/puzzles/daily] missing FEN; using fallback"
        )
        return FALLBACK_PUZZLE
    ...
    if not san:
        log.warning(
            "[/puzzles/daily] no SAN found; using fallback"
        )
        return FALLBACK_PUZZLE
    ...
    try:
        ...
        log.error(
            "[/puzzles/daily] SAN->UCI failed; using fallback: %s",
            e,
        )
        return FALLBACK_PUZZLE
    ...
    if not moves:
        log.warning(
            "[/puzzles/daily] empty UCI after convert; using fallback"
        )
        return FALLBACK_PUZZLE
    ...
    puzzle_id = (
        ...
        or j.get("title")
        or j.get("url")
        or f"chesscom-{j.get('publish_time', 'na')}"
    )
...
async def puzzles_random():
    try:
        ...
        if r.status_code != 200:
            raise HTTPException(
                status_code=502,
                detail=f"chess.com {r.status_code}",
            )
        ...
        raise HTTPException(
            status_code=502,
            detail=f"random puzzle fetch failed: {e}",
        )

    if not j.get("fen") or (
        not j.get("moves") and not j.get("pgn")
    ):
        raise HTTPException(
            status_code=502,
            detail="chess.com random: missing fen/moves",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"random SAN->UCI failed: {e}",
        )
...
async def puzzles_random_ml():
    ...
    if not rows:
        raise HTTPException(
            status_code=404,
            detail="no ML-scored puzzles available in band",
        )
...
async def puzzles_page():
    ...
    if not html_path.exists():
        raise HTTPException(
            status_code=404,
            detail="puzzles.html not found in public/",
        )
...
async def puzzles_from_game(body: FromGameRequest):
    """
    Generate puzzles from ONE selected game.
    """

    try:
        max_puzzles = min(
            max(int(body.maxPuzzles or 12), 1),
            50,
        )
...
@router.post("/puzzles/analyze-all")
async def puzzles_analyze_all(body: FromUserMlRequest):
    """
    Analyze the user's recent games and generate
    ML-ranked puzzles across all of them.
    """
    ...
    max_puzzles = min(
        max(int(body.maxPuzzles or 100), 1),
        500,
    )

    movetime_ms = min(
        max(int(body.movetimeMs or 40), 20),
        200,
    )
